...
import {
  FaUsers, FaFileInvoiceDollar, FaMoneyCheckAlt, FaProjectDiagram,
  FaChartLine, FaFileAlt, FaFileContract, FaSyncAlt
} from "react-icons/fa";
...
const Home = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const requests = [
        axios.get("http://localhost:5000/api/client/invoices", config),
        axios.get("http://localhost:5000/api/client/payments", config),
        axios.get("http://localhost:5000/api/client/projects", config),
        axios.get("http://localhost:5000/api/client/proposals", config),
        axios.get("http://localhost:5000/api/client/estimates", config),
        axios.get("http://localhost:5000/api/client/contacts", config)
      ];
      ...
      if (responses[0].status === 'fulfilled') {
        ...
        const totalAmount = invoicesData.reduce((sum, invoice) => sum + (invoice.total || 0), 0);
        ...
        const recentInvoices = invoicesData
          ...
          .map(invoice => ({
            type: 'invoice',
            title: invoice.invoiceNumber || `INV-${invoice._id.slice(-6).toUpperCase()}`,
      ...
      if (responses[1].status === 'fulfilled') {
        ...
        const totalPaymentAmount = paymentsData.reduce((sum, payment) => sum + (payment.amount || 0), 0);
      ...
      if (responses[2].status === 'fulfilled') {
        ...
        setStats(prev => ({
          ...
          projects: {
            ...
            progress: projectsData.filter(p => p.status === "Progress" || p.status === "progress").length,
            onHold: projectsData.filter(p => p.status === "On Hold" || p.status === "onHold").length,
            cancelled: projectsData.filter(p => p.status === "Cancelled" || p.status === "cancelled").length,
            finished: projectsData.filter(p => p.status === "Finished" || p.status === "finished").length
          }
        }));
      ...
      if (responses[3].status === 'fulfilled') {
        ...
        setStats(prev => ({
          ...
          proposals: {
            ...
            draft: proposalsData.filter(p => p.status === "Draft" || p.status === "draft").length,
            sent: proposalsData.filter(p => p.status === "Sent" || p.status === "sent").length,
            open: proposalsData.filter(p => p.status === "Open" || p.status === "open").length,
            revised: proposalsData.filter(p => p.status === "Revised" || p.status === "revised").length,
            declined: proposalsData.filter(p => p.status === "Declined" || p.status === "declined").length,
            accepted: proposalsData.filter(p => p.status === "Accepted" || p.status === "accepted").length
          }
        }));
      }
      ...
      if (responses[4].status === 'fulfilled') {
        ...
        setStats(prev => ({
          ...
          estimates: {
            ...
            draft: estimatesData.filter(e => e.status === "Draft" || e.status === "draft").length,
            pending: estimatesData.filter(e => e.status === "Pending" || e.status === "pending").length,
            approved: estimatesData.filter(e => e.status === "Approved" || e.status === "approved").length,
            rejected: estimatesData.filter(e => e.status === "Rejected" || e.status === "rejected").length,
            expired: estimatesData.filter(e => e.status === "Expired" || e.status === "expired").length
          }
        }));
      }
      ...
      if (responses[5].status === 'fulfilled') {
        ...
        const recentContacts = contactsData
          ...
          .map(contact => ({
            type: 'contact',
            title: contact.subject || `Contact ${contact._id.slice(-6).toUpperCase()}`,
            status: 'active',
    ...
    <div className="bg-gray-50 min-h-screen p-6">
      <div className="w-full mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-800 mb-2">Client Home</h1>
          </div>
          <button 
          ...
          <div className="bg-white rounded-lg shadow p-4 border border-gray-200 hover:shadow-md transition-shadow">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-gray-800">{formatNumber(stats.payments.total)}</p>
              </div>
            </div>
            <div className="mt-2 text-xs text-gray-500">
          ...
          <div className="bg-white rounded-lg shadow p-4 border border-gray-200 hover:shadow-md transition-shadow">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-gray-800">{formatNumber(stats.contacts.total)}</p>
              </div>
            </div>
            <div className="mt-2 text-xs text-gray-500">
              Active contacts
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={paymentHistoryData}>
                      ...
                      <Legend />
                      <Bar dataKey="amount" name="Payment Amount" fill="#3B82F6" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                  ...
                  {paymentHistoryData.slice(0, 3).map((payment, index) => (
                    <div key={index} className="flex justify-between items-center text-sm">
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-gray-500 text-xs ml-2">{payment.date}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {recentActivity.map((activity, index) => (
                  <div key={index} className="flex items-start">
                    <div className={`p-2 rounded-full mr-3 ${
                      ...
                      activity.type === 'contact' ? 'bg-gray-100 text-gray-600' :
                      'bg-purple-100 text-purple-600'
                    }`}>
                      {activity.type === 'invoice' ? <FaFileInvoiceDollar className="h-4 w-4" /> :
                       activity.type === 'payment' ? <FaMoneyCheckAlt className="h-4 w-4" /> :
                       activity.type === 'contact' ? <FaUsers className="h-4 w-4" /> :
                    ...
                    <div className={`px-2 py-1 rounded text-xs ${
                      activity.status === 'paid' || activity.status === 'completed' || activity.status === 'finished' || activity.status === 'active' ? 'bg-green-100 text-green-800' :
                      activity.status === 'unpaid' || activity.status === 'cancelled' ? 'bg-red-100 text-red-800' :
                      activity.status === 'progress' ? 'bg-blue-100 text-blue-800' :
                      activity.status === 'partiallypaid' ? 'bg-yellow-100 text-yellow-800' :
        ...
        <div className="bg-white rounded-lg shadow p-6 border border-gray-200 mb-8">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Portal Overview</h2>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
