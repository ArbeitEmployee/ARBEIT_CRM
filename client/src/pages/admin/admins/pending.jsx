...
import { FaEye, FaCheck, FaTimes, FaTrash, FaExclamationTriangle } from "react-icons/fa";
...
const PendingAdmins = () => {
  ...
  const fetchPendingAdmins = async () => {
    try {
      ...
      setError(error.message || "Failed to fetch pending admins. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApproveAdmin = async (adminId) => {
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to approve admin: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const handleRejectAdmin = async (adminId) => {
    if (!window.confirm("Are you sure you want to reject and delete this admin?")) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        alert(`Failed to reject admin: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
          ...
          <table className="w-full text-sm border-separate border-spacing-y-2">
            <thead>
              <tr className="text-left">
                ...
                <th className="p-3" style={{ backgroundColor: '#333333', color: 'white' }}>Request Date</th>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg">
            ...
            <div className="space-y-3">
              ...
              <p><b>Status:</b> <span className="px-2 py-1 rounded text-xs bg-yellow-100 text-yellow-800">
                {viewAdmin.status}
              </span></p>
              <p><b>Request Date:</b> {new Date(viewAdmin.createdAt).toLocaleString()}</p>
            </div>
