...
const ExpensesReport = () => {
  ...
  const fetchExpenses = async () => {
    ...
    try {
      ...
      const years = [...new Set(data.expenses.map(expense => {
    ...
    expenses.forEach(expense => {
      ...
      if (dateParts.length === 3) {
        ...
        if (expenseYear === year && month >= 0 && month < 12) {
          ...
          const categoryIndex = monthlyExpenses.findIndex(item => item.category === expense.category);
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-4">
      ...
      <div className="bg-white shadow-md rounded p-4 mb-6">
        <h2 className="text-xl font-semibold mb-4">Monthly Expenses by Category ({selectedYear})</h2>
            ...
            <tbody>
              {monthlyData.map((categoryData, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                  ...
                  <td className="p-3 text-right">{formatCurrency(categoryData.January || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.February || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.March || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.April || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.May || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.June || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.July || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.August || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.September || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.October || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.November || 0)}</td>
                  <td className="p-3 text-right">{formatCurrency(categoryData.December || 0)}</td>
                  <td className="p-3 text-right font-bold">{formatCurrency(categoryData.total || 0)}</td>
                </tr>
              ))}
              {/* Total row */}
              <tr className="bg-gray-100 font-bold">
                ...
                {['January', 'February', 'March', 'April', 'May', 'June', 
                  ...
                  <td key={month} className="p-3 text-right">
                    {formatCurrency(monthlyData.reduce((sum, category) => sum + (category[month] || 0), 0))}
                  </td>
                ))}
                <td className="p-3 text-right">
                  {formatCurrency(monthlyData.reduce((sum, category) => sum + (category.total || 0), 0))}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white shadow-md rounded p-4">
          <h3 className="text-lg font-semibold mb-4">Billable Expenses by Month ({selectedYear})</h3>
          <div className="h-64">
        ...
        <div className="bg-white shadow-md rounded p-4">
          <h3 className="text-lg font-semibold mb-4">Non-Billable Expenses by Month ({selectedYear})</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={nonBillableData}>
                ...
                <Tooltip formatter={(value) => [formatCurrency(value), 'Amount']} />
                <Legend />
                <Bar dataKey="amount" name="Non-Billable Amount" fill="#EF4444" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="bg-white shadow-md rounded p-4 mb-6">
        <h3 className="text-lg font-semibold mb-4">Expenses by Category ({selectedYear})</h3>
        <div className="h-96">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              data={monthlyData}
