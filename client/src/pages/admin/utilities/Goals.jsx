...
const GoalsPage = () => {
  ...
  const fetchGoals = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${API_BASE_URL}/admin/goals`, {
        params: {
          search: searchTerm,
  ...
  const handleSaveGoal = async () => {
    ...
    if (
      ...
      alert(
        "Please fill in all required fields (Title, Goal Type, Target Value, Start Date, End Date)"
      );
      return;
    }
    ...
    try {
      ...
      if (editingGoal) {
        ...
        await axios.put(
          `${API_BASE_URL}/admin/goals/${editingGoal._id}`,
          goalData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        // Create new goal
        await axios.post(`${API_BASE_URL}/admin/goals`, goalData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
      ...
      alert(
        `Error saving goal: ${error.response?.data?.message || error.message}`
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const handleDeleteGoal = async (id) => {
    if (window.confirm("Are you sure you want to delete this goal?")) {
      try {
        const token = getAuthToken();
        await axios.delete(`${API_BASE_URL}/admin/goals/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="mb-4">
              ...
              >
                ...
                <option value="Make Contracts By Type">
                  Make Contracts By Type
                </option>
                <option value="Achieve Total Income">
                  Achieve Total Income
                </option>
                <option value="Increase Customer Number">
                ...
                {selectedGoals.length > 0 && (
                  <button
                    ...
                    onClick={async () => {
                      if (
                        window.confirm(
                          `Delete ${selectedGoals.length} selected goals?`
                        )
                      ) {
                        try {
                          const token = getAuthToken();
                          await axios.post(
                            `${API_BASE_URL}/admin/goals/bulk-delete`,
                            {
                  ...
                  <tr className="text-left">
                    <th className="p-3 rounded-l-lg bg-gray-900 text-white">
                      <input
                        type="checkbox"
                        checked={
                          selectedGoals.length === currentData.length &&
                          currentData.length > 0
                        }
                        onChange={(e) => {
                          if (e.target.checked) {
                            setSelectedGoals(
                              currentData.map((goal) => goal._id)
                            );
                          } else {
                            setSelectedGoals([]);
                          }
